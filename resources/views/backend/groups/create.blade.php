...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800 font-weight-bold"><i class="fas fa-layer-group text-primary mr-2"></i>Add New Group
        </h1>
            ...
            <form action="{{ route('admin.groups.store') }}" method="POST">
                    ...
                    <select name="competition_id" class="form-control @error('competition_id') is-invalid @enderror"
                        ...
                        @foreach ($competitions as $competition)
                            <option value="{{ $competition->id }}"
                                {{ old('competition_id') == $competition->id ? 'selected' : '' }}>{{ $competition->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input name="name" value="{{ old('name') }}"
                        class="form-control @error('name') is-invalid @enderror" type="text" placeholder="e.g. Group C"
                        required>
                    @error('name')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
